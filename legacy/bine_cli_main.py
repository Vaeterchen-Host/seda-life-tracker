...
"""Alternative CLI controller prototype for SEDA."""
# pylint: disable=all
...
def get_current_user_object(db_user_row):
    ...
    return User(
        ...
        weight=[],
        food=[],
        meal=[],  # starts off empty and will be filled as needed
    )
...
if not all_users_raw:
    ...
    db.add_user(
        temp_user.name,
        temp_user.birthdate,
        temp_user.height_in_cm,
        temp_user.gender,
        temp_user.fitness_lvl,
...
current_user = get_current_user_object(
    all_users_raw[0]
)  # Assuming we take the first user for now; can be expanded to allow user selection

ui.cli_view.show_message(
    f"Logged in as: {current_user.name} (ID: {current_user.user_id})"
)
...
while True:
    ...
    if choice == "1":
        ui.cli_view.show_user_info(
            current_user
        )  # retrieving data from the current user object
        ...
        db_id = db.add_water_log(
            current_user.user_id, water_log_1.amount_in_ml, water_log_1.timestamp
        )
        ...
        # Add a weight log
        weight_in_kg, timestamp = (
            ui.cli_view.create_weight_log_by_input()
        )  # zum Abfragen der Daten über die CLI View
        weight_log_1 = current_user.add_weight_log(weight_in_kg, timestamp)
        db_id = db.add_weight_log(
            current_user.user_id, weight_log_1.weight_in_kg, weight_log_1.timestamp
        )
        ...
        user_logs_raw = [
            log for log in raw_logs if log[1] == current_user.user_id
        ]  # Filter logs for the current user
        weight_log_objects = [
            WeightLog(id=log[0], weight_in_kg=log[3], timestamp=log[4])
            for log in user_logs_raw
        ]  # Convert raw logs to WeightLog objects
        ...
        if weight_log_objects:
            log_id_to_delete = int(
                input("Enter the ID of the weight log you want to delete: ")
            )
            db.delete_weight_log(
                log_id_to_delete
            )  # pylint: disable=no-value-for-parameter
            current_user.delete_weight_log(
                log_id_to_delete
            )  # deleting from the local object as well
